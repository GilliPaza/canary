-- Entrance lever for the Marapur Turtles instanced hunt, on a dry patch at the
-- western edge of the real reef (Position(33675-33677, 32758-32766, 8)). Group
-- entry: whoever is standing on config.leverPlatformTiles when the lever is used
-- enters together (up to config.groupSize). See instanced_hunts_config_marapur_turtles.lua.

local config = InstancedHuntsConfigs.MarapurTurtles
local marapurTurtlesLever = Action()
...
function marapurTurtlesLever.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	for _, p in ipairs(group) do
		p:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You have entered a private Marapur Turtles hunt. You have " .. InstancedHunts.formatDuration(InstancedHunts.getBudgetRemaining(p)) .. " of hunting time left.")
	end
...
marapurTurtlesLever:id(8911, 8912, 8913, 8914)
marapurTurtlesLever:aid(config.leverActionId)
marapurTurtlesLever:register()
